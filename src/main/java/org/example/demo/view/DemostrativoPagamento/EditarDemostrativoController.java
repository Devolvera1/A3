...
public class EditarDemostrativoController {
    ...
    public void setDados(Departamento.HistoricoSalario registro) {
        Platform.runLater(() -> {
            ID.setText(String.valueOf(registro.getId()));
            Salario.setText(String.valueOf(registro.getSalario()));
            DataInicio.setText(formatarDataTela(registro.getDataInicio()));
            DataFim.setText(formatarDataTela(registro.getDataFim()));
            Motivo.setValue(registro.getMotivo());
    ...
    private String formatarDataTela(String d) {
    ...
    private void Ok(ActionEvent event) {
        String sql = "UPDATE historIcoSalarios  SET funcionario_id=?, salario=?, dataInicio=?, dataFim=?, motivo=? WHERE id=?";
        try (Connection conn = Database.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setInt(1, Funcionarios.getValue().getId());
            pstmt.setDouble(2, Double.parseDouble(Salario.getText().replace(",", ".")));
            pstmt.setString(3, converterDataBanco(DataInicio.getText()));
            pstmt.setString(4, converterDataBanco(DataFim.getText()));
    ...
    private String converterDataBanco(String d) {
